...
if [ ! -n "$3" ] ;then
    RESULT_DIR="./results/Qwen/IconQA"
else
    RESULT_DIR=$3
fi
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m ETrain.Eval.Qwen.eval_vllm \
        ...
        --model-base ./checkpoints/Qwen/Qwen2.5_VL \
        --question-file ./playground/Instructions_Light/IconQA/test_3000.json \
...
python -m ETrain.Eval.Qwen.general_eval \
    --annotation-file ./playground/Instructions_Light/IconQA/test_3000.json \
